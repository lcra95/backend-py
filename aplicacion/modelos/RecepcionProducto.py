...
class RecepcionProducto(db.Model):
    __tablename__ = 'recepcion_producto'


    id = db.Column(db.Integer, primary_key=True)
    id_producto = db.Column(db.Integer, nullable=False)
    id_recepcion = db.Column(db.Integer, nullable=False)
    cantidad = db.Column(db.Integer, nullable=False)
    precio_neto = db.Column(db.String(128), nullable=False)
    ...
    def update_data(cls, _id, dataJson):
        try:
            ...
            if query:
                ...
                if 'id_recepcion' in dataJson:
                    query.id_recepcion = dataJson['id_recepcion']
                if 'cantidad' in dataJson:
                    query.cantidad = dataJson['cantidad']
                if 'precio_neto' in dataJson:
                    query.precio_neto = dataJson['precio_neto']
