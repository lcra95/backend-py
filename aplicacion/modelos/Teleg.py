...
class Telegram(db.Model):
    ...
    id_update = db.Column(db.String(90), nullable=False)
    id_chat = db.Column(db.String(90), nullable=False)
    ...
    def get_data(cls, _id):
        query =  cls.query.filter_by(id_update=_id).first()
        return  Utilidades.obtener_datos(query)

    @classmethod
    def insert(cls, dataJson):
        query = Telegram( 
                id_update = dataJson["id_update"],
                id_chat = dataJson["id_chat"],
                # created_at = func.NOW(),
                # updated_at = func.NOW()
            )
        Telegram.guardar(query)
        if query.id:                            
            return  query.id 
        return  False
    ...
    def update_data(cls, _id, dataJson):
        try:
            ...
            if query:
                if "id_update" in dataJson:
                    query.id_update = dataJson["id_update"]
               
                if "id_chat" in dataJson:
                    query.id_chat = dataJson["id_chat"]
